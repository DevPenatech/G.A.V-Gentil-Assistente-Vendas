...
def detect_quantity_modifiers(text: str) -> Dict:
    ...
    normalized = normalize_text(text)
    modifiers = {
        'action': None,  # 'add', 'set', 'remove', 'clear
    ...
    elif re.search(r'\b(?:esvaziar|limpar|zerar|resetar|apagar)\s*(?:carrinho|tudo|todos|completo)?', normalized):
        modifiers['action'] = 'clear'
        modifiers['reference'] = 'all'
    # NOVO: Comandos alternativos para limpeza
    elif re.search(r'\b(?:começar\s+de\s+novo|recomeçar|reiniciar)', normalized):
        modifiers['action'] = 'clear'
        modifiers['reference'] = 'all'
    
    # Referências - EXPANDIDO
    if re.search(r'\b(?:tudo|todos|todas|carrinho|completo|inteiro|total)\b', normalized):
        modifiers['reference'] = 'all'
